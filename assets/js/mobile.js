...
function addData(arg,callback){
    ...
    _this.prototype={
        ...
        ajax:function(arg,_page){
            var _obj=arg.mainbox.next('.loading_more'),_data=$.extend(arg.res,{pageNo:_page});
                $.ajax({
                url:arg.url,
                type:'post',
                dataType:'json',
                data:_data,
                beforeSend: function () {_obj.remove();arg.mainbox.after('<div class="loading_more"><svg class="load-icon" style="margin-right:10px;" width="30" height="30" viewBox="0,0,130,130"><circle cx="65" cy="65" r="61" stroke="#d9d9d9" fill="none" stroke-width="4"></circle><path class="path" id="path" stroke="#fed369" fill="none" stroke-width="4" d="M 65 4 A 61 61, 0, 1, 1, 9.075676085688478 40.638760402492565"></path></svg>数据加载中...</div>');_obj=$(".loading_more");},
                success:function(result){
                    arg.success(result,arg.mainbox,arg.isrefresh);
                    arg.isrefresh=false;
                    if(!result.totalCount){
                        _obj.html('<div class="gray">抱歉！暂无数据</div>')
                    } else{
                        maxnum = Math.ceil(result.totalCount/result.pageSize);
                        if(this_page<maxnum){
                            _obj.html('<svg class="load-icon" style="margin-right:10px;" width="30" height="30" viewBox="0,0,130,130"><circle cx="65" cy="65" r="61" stroke="#d9d9d9" fill="none" stroke-width="4"></circle><path class="path" id="path" stroke="#fed369" fill="none" stroke-width="4" d="M 65 4 A 61 61, 0, 1, 1, 9.075676085688478 40.638760402492565"></path></svg>数据加载中...');
                        }else{
